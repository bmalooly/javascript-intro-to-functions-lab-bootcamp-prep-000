function shout(string){return string.toUpperCase()}
function whisper(string){return string.toLowerCase()}
function logShout(string){console.log(string.toUpperCase('hello'))}
function logWhisper(string){console.log(string.toLowerCase('HELLO'))}
function sayHitoGrandma(string){if(string.toLowerCase)() return ("I can/'t hear you!")} else if(string.toUpperCase) return("YES INDEED!")} else (string==="I love you, Grandma")